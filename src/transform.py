...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    data = pd.merge(items, orders, left_on="order_id", right_on="order_id")
    data = pd.merge(data, products, left_on="product_id", right_on="product_id")
    ...
    delivered = data[(data["order_status"] == "delivered") & (data["order_delivered_customer_date"].notnull())]
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date["holiday"] = order_purchase_ammount_per_date["order_purchase_timestamp"].isin(holidays["date"])
    # Renombrar columna de fecha
    order_purchase_ammount_per_date = order_purchase_ammount_per_date.rename(columns={"order_purchase_timestamp": "date"})
